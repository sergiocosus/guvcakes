...
class VideoCategoriaControlador extends BaseControlador{ 
    public  $modelo = "VideoCategoria";
    public  $modeloCat = "VideoCategoria";
    public  $imageDir='/imagenes/VideoCategoria/';
    
    function accionVer(){
        include_once "Modelos/$this->modelo.php";
        include_once 'Vistas/VideoCategoria/ver.php';
    }
    
    function accionEditar(){
        sesionIniciada();
        include_once 'Modelos/VideoCategoria.php';
        global $urlArray,$urlArray;
        if(count($urlArray)===2){
            $categorias=VideoCategoria::obtenerTodos('where visible=1');
        }
        include_once 'Vistas/VideoCategoria/editar.php';
    }
    
    
    function accionObtenerHuerfanos(){
        include_once "Modelos/$this->modelo.php";
        $categorias=VideoCategoria::obtenerTodos();
        $catIds=array();
        for($i=0;$i<count($categorias);$i++){
            array_push($catIds,$categorias[$i]->id);
        }
        array_push($catIds,0);
        $categorias=VideoCategoria::obtenerTodos('where visible=1 and catId!='.implode(' and catId!=',$catIds));
        VideoCategoria::codificarArreglo($categorias);
    }
