...
class VideoCategoria extends Base{
    public static $tabla = "video_categorias";
    public $id;
    public $link;
    public $titulo;
    public $descripcion;
    ...
    static function codificarArreglo($categorias){
        ...
        if(count($categorias)>0)
            static::codificarVideoCategoriaSimple($categorias[0]);
        for($i=1;$i<count($categorias);$i++){
            ?>,<?php static::codificarVideoCategoriaSimple($categorias[$i]) ?>
        <?php }
        echo ']';       
    }
    
    static function codificarVideoCategoriaSimple($categoria){
        ?>VideoCategoria.simple(<?php echo $categoria->id; ?>,<?php echo json_encode($categoria->link); ?>,<?php echo json_encode($categoria->titulo); ?>,<?php echo json_encode($categoria->descripcion); ?>,<?php echo json_encode($categoria->formato); ?>,<?php echo $categoria->catId; ?>,<?php echo $categoria->contiene; ?>)<?php
    }
     
    static function codificar($categoria){
        ?>new VideoCategoria(<?php echo $categoria->id;
