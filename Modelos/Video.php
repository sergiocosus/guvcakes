...
class Foto extends Base{
    ...
    static function codificarArreglo($fotos){
        echo '[';
        if(count($fotos)>0)
            static::codificarVideoSimple($fotos[0]);
        for($i=1;$i<count($fotos);$i++){
            ?>,
            <?php static::codificarVideoSimple($fotos[$i]) ?><?php }
        echo '];';
    }
    
    static function codificarVideoSimple($foto){
        ?>Video.simple(<?php echo $foto->id; ?>,
